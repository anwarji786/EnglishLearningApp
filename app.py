...
def show_quiz_results(questions: List[Dict], answers: List[Dict], engine: LearningEngine):
    ...
    with st.expander("📋 Review Answers"):
        for i, (question, answer) in enumerate(zip(questions, answers), 1):
            ...
            # Audio for the word - FIXED: use engine.audio_manager instead of audio_manager
            audio_bytes = engine.audio_manager.generate_audio(question['word'].english)
            if audio_bytes:
                engine.audio_manager.render_player(audio_bytes, question['word'].english, f"review_{question['word'].english}")
