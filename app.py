...
import os
import glob
import uuid  # <-- Added for UUID-based audio IDs
...
# ============================================================================
class AudioManager:
    """Handles all audio operations with caching and single-audio enforcement"""
    def __init__(self):
    ...
    def create_audio_player(self, audio_bytes: bytes, text: str = "", context: str = "") -> None:
        """Render a single audio player that auto-pauses others and never loops"""
        if not audio_bytes:
            return

        # Generate a truly unique ID using UUID to avoid collisions across reruns
        ...
        # JavaScript to enforce single-audio playback and reset state
        js_code = f"""
        <script>
        (function() {{
        ...
        # Render clean audio player WITHOUT loop button or status
        html_code = f"""
...
def render_word_details(word: WordData, audio_manager: AudioManager):
    """Word details display with clean audio player (no loop)"""
...
cols_per_row = 4
words = story['content']
# Create rows using global index to ensure unique keys
for global_idx in range(0, len(words), cols_per_row):
    ...
    for col_offset, word_data in enumerate(row):
        with cols[col_offset]:
            ...
            # Use global word index to guarantee uniqueness (fixes StreamlitDuplicateElementKey)
            unique_key = f"word_{story_idx}_{global_idx + col_offset}_{word_data.english}"
            if st.button(
