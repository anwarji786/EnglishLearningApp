...
class AudioManager:
    ...
    def render_player(self, placeholder, audio_bytes: bytes, label: str, word_text: str):
        """
        Renders HTML5 audio player with strict single-audio enforcement.
        """
        if not audio_bytes:
            placeholder.empty()
            return
        
        # Generate unique IDs for this specific word/instance
        unique_id = f"audio_{uuid.uuid4().hex[:8]}"
        session_audio_key = f"active_audio_{hashlib.md5(word_text.encode()).hexdigest()[:8]}"
        
        # Store active audio ID in session state for cross-rerun tracking
        st.session_state[session_audio_key] = unique_id
        
        b64_audio = base64.b64encode(audio_bytes).decode()
        
        # CRITICAL: Add cache-busting timestamp to force fresh audio load
        timestamp = int(time.time() * 1000)
        
        html = f"""
        <script>
        // SINGLE-AUDIO ENFORCEMENT: Pause all other audio elements
        function enforceSingleAudio(currentId) {{
            document.querySelectorAll('audio').forEach(audio => {{
                if (audio.id !== currentId && !audio.paused) {{
                    audio.pause();
                    audio.currentTime = 0;
                    // Remove any existing loop handlers
                    audio.onended = null;
                }}
            }});
        }}
        
        // SAFE LOOP CONTROL with proper cleanup
        function setupSafeLoop(audioId) {{
            ...
            // Set new handler with proper closure
            audio.onended = function() {{
                // Verify this is still the current audio element
                if (audio.id === "{unique_id}") {{
                    audio.currentTime = 0;
                    audio.play();
                }}
            }};
        ...
        // Initialize for this audio element
        window.addEventListener('DOMContentLoaded', function() {{
            const audio = document.getElementById("{unique_id}");
            if (audio) {{
                // Ensure fresh load with cache busting
                audio.load();
                
                // Set up single-audio enforcement on play
                audio.onplay = function() {{
                    enforceSingleAudio("{unique_id}");
                }};
        ...
        <div style="background: #f1f3f5; padding: 15px; border-radius: 10px; margin: 15px 0; border: 1px solid #dee2e6;">
            ...
            <!-- Cache bust with timestamp AND unique fragment -->
            <audio id="{unique_id}" controls style="width: 100%; height: 40px; margin-bottom: 10px;"
                   data-word="{word_text}" data-timestamp="{timestamp}">
            ...
            <div style="display: flex; gap: 8px; margin-top: 10px;">
                <button onclick="
                    ...
                    enforceSingleAudio('{unique_id}');
                    aud.play();
                " style="flex:1; background:#4CAF50; color:white; border:none; padding:10px; border-radius:5px; font-size:1rem;">
                    ▶ Play
                </button>
                <button onclick="
                    const aud = document.getElementById('{unique_id}');
                    ...
                    aud.currentTime = 0;
                    aud.onended = null;
                " style="flex:1; background:#FF9800; color:white; border:none; padding:10px; border-radius:5px; font-size:1rem;">
                    ⏸ Pause
                </button>
                <button onclick="
                    const aud = document.getElementById('{unique_id}');
                    ...
                    aud.currentTime = 0;
                    aud.play();
                    setupSafeLoop('{unique_id}');
                " style="flex:1; background:#2196F3; color:white; border:none; padding:10px; border-radius:5px; font-size:1rem;">
                    🔁 Loop
                </button>
            </div>
        </div>
        ...
        # Force complete refresh of the placeholder
        placeholder.empty()
...
def mode_story_reader(story_data: List[WordData], story_filename: str, audio_mgr: AudioManager, storage: StorageManager, profile: UserProfile):
    ...
    # Init State with audio tracking
    if 'reader_filename' not in st.session_state or st.session_state['reader_filename'] != story_filename:
        ...
        # Clear any audio tracking when switching stories
        st.session_state.last_audio_word = None
    ...
    # AUDIO STATE RESET: Clear previous audio placeholders when navigating
    if 'last_audio_word' in st.session_state and st.session_state.last_audio_word != idx:
        # This ensures old audio elements get garbage collected
        st.session_state.last_audio_word = idx
    ...
    with col_prev:
        if st.button("⬅️ Prev", disabled=(idx == 0), use_container_width=True, key="btn_prev"):
            # Force audio cleanup before navigation
            st.session_state.reader_idx -= 1
            st.session_state.show_hindi = False
            # Clear any pending audio state
            if 'last_audio_word' in st.session_state:
                del st.session_state.last_audio_word
            st.rerun()
    ...
    # Create fresh placeholder - old one will be garbage collected
    audio_placeholder = st.empty()
    
    # Store current word index for audio tracking
    st.session_state.last_audio_word = idx
