...
# ============================================================================
# FIXED AUDIO MANAGER WITH SINGLE-AUDIO ENFORCEMENT
# ============================================================================
...
class AudioManager:
    ...
    def render_player(self, audio_bytes: bytes, text: str = "", player_key: str = "") -> None:
        """Create a single audio player with enforced single-audio playback"""
        if not audio_bytes:
            return
        
        # Generate unique ID for this audio instance
        audio_hash = hashlib.md5(audio_bytes).hexdigest()[:12]
        current_counter = st.session_state.audio_state['audio_counter'] + 1
        st.session_state.audio_state['audio_counter'] = current_counter
        
        # Create unique audio ID with counter to ensure uniqueness
        audio_id = f"audio_{audio_hash}_{current_counter}"
        ...
        # JavaScript to enforce single-audio playback
        js_code = f"""
        <script>
        function setupAudio_{audio_id}() {{
            // Get all audio elements
            const allAudios = document.querySelectorAll('audio');
            
            // Pause all other audio elements
            allAudios.forEach(audio => {{
                if (audio.id !== 'audio_{audio_id}') {{
                    audio.pause();
                    audio.currentTime = 0;
            ...
            // Add play handler to pause others
            ...
            audio.onplaying = function() {{
                const status = document.getElementById('status_{audio_id}');
                if (status) {{
        ...
        if (document.readyState === 'loading') {{
            document.addEventListener('DOMContentLoaded', function() {{
                setupAudio_{audio_id}();
            }});
        }} else {{
            setupAudio_{audio_id}();
        }}
        
        // Also setup when element is added dynamically
        const observer = new MutationObserver(function(mutations) {{
            if (document.getElementById('audio_{audio_id}')) {{
                setupAudio_{audio_id}();
                observer.disconnect();
            }}
        }});
        ...
        # Store current audio ID in session state
        st.session_state.audio_state['active_audio_id'] = audio_id
        st.session_state.audio_state['last_audio_hash'] = audio_hash
    ...
    def create_audio_player(self, audio_bytes: bytes, text: str = "", audio_id: str = "") -> None:
        """Wrapper for backward compatibility - uses new render_player method"""
...
def render_word_details(word: WordData, audio_manager: AudioManager):
    ...
    with col2:
        ...
        if audio_bytes:
            ...
            audio_manager.render_player(audio_bytes, word.english, f"word_{word.english}")
        ...
        if word.example_sentence:
            ...
            if sent_audio:
                audio_manager.render_player(sent_audio, word.example_sentence, f"sentence_{word.english}")
...
def render_flashcards(review_words: List[WordData], audio_manager: AudioManager, engine: LearningEngine):
    ...
    if audio_bytes:
        audio_manager.render_player(audio_bytes, current_word.english, f"flashcard_{current_word.english}")
    ...
    if not session['show_answer']:
        ...
        if current_word.example_sentence:
            ...
            if sent_audio:
                audio_manager.render_player(sent_audio, current_word.example_sentence, f"flashcard_sentence_{current_word.english}")
...
def render_quiz_session(review_words: List[WordData], audio_manager: AudioManager, engine: LearningEngine):
    ...
    if audio_bytes:
        audio_manager.render_player(audio_bytes, question['word'].english, f"quiz_{question['word'].english}")
...
def show_quiz_results(questions: List[Dict], answers: List[Dict], engine: LearningEngine):
    ...
    with st.expander("📋 Review Answers"):
        for i, (question, answer) in enumerate(zip(questions, answers), 1):
            ...
            if audio_bytes:
                audio_manager.render_player(audio_bytes, question['word'].english, f"review_{question['word'].english}")
...
def main():
    ...
    # IMPORTANT NOTICE - Updated for working audio player
    st.markdown("""
    <div class="important-note">
    🔊 <strong>Audio Player Now Working!</strong><br>
    1. Click any word to learn it<br>
    2. Use the audio player to listen<br>
    3. <strong>NEW:</strong> Only one audio plays at a time - previous audio automatically stops<br>
    <em>Audio desynchronization issues are now fixed!</em>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.get('test_audio_triggered', False):
        ...
        if test_audio:
            audio_manager.render_player(test_audio, "Test Audio", "test_audio")
            st.success("✅ Audio is working! Try playing multiple audio players - only one will play at a time.")
    ...
    if st.button("🎧 Listen to Full Story"):
        ...
        if full_audio:
            audio_manager.render_player(full_audio, story_text_english, f"story_{story_idx}")
        ...
        for i, word_data in enumerate(row):
            with cols[i]:
                ...
                if st.button(
                    ...
                    key=f"word_{story_idx}_{word_data.english}_{i}",
    ...
    st.markdown("---")
    with st.expander("📚 Browse All Words (with audio)"):
        ...
        for i, word in enumerate(sorted(filtered_words, key=lambda w: w.english)):
            ...
            with col4:
                ...
                audio_bytes = audio_manager.generate_audio(word.english)
                if audio_bytes:
                    audio_manager.render_player(audio_bytes, word.english, f"browse_{word.english}_{i}")
