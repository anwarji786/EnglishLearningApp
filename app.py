...
# ============================================================================
# ✅ FINAL AUDIO MANAGER — USES st.audio() FOR 100% RELIABLE PLAYBACK
# ============================================================================
class AudioManager:
    """Handles all audio operations using Streamlit's native audio player"""
    def __init__(self):
    ...
    def create_audio_player(self, audio_bytes: bytes, label: str, audio_id: str):
        """✅ Use Streamlit's native st.audio() — guaranteed to work"""
        if not audio_bytes:
...
# ============================================================================
# UI COMPONENTS (NO CHANGE NEEDED)
# ============================================================================
...
def render_word_details(word: WordData, audio_manager: AudioManager):
    ...
    with col2:
        ...
        if audio_bytes:
            audio_manager.create_audio_player(audio_bytes, word.english, "")
        # Sentence audio
        if word.example_sentence:
            st.markdown("**Sentence:**")
            sent_audio = audio_manager.generate_audio(word.example_sentence, slow=False)
            if sent_audio:
                audio_manager.create_audio_player(sent_audio, word.example_sentence, "")
...
def render_flashcards(review_words: List[WordData], audio_manager: AudioManager, engine: LearningEngine):
    ...
    if audio_bytes:
        audio_manager.create_audio_player(audio_bytes, current_word.english, "")
    ...
    if not session['show_answer']:
        ...
        if current_word.example_sentence:
            ...
            if sent_audio:
                audio_manager.create_audio_player(sent_audio, current_word.example_sentence, "")
...
def render_quiz_session(review_words: List[WordData], audio_manager: AudioManager, engine: LearningEngine):
    ...
    if audio_bytes:
        audio_manager.create_audio_player(audio_bytes, question['word'].english, "")
...
def show_quiz_results(questions: List[Dict], answers: List[Dict], engine: LearningEngine):
    ...
    with st.expander("📋 Review Answers"):
        for i, (question, answer) in enumerate(zip(questions, answers), 1):
            ...
            if audio_bytes:
                engine.audio_manager.create_audio_player(audio_bytes, question['word'].english, "")
            st.markdown("---")
    col1, col2 = st.columns(2)
    with col1:
...
def main():
    ...
    if st.session_state.get('test_audio_triggered', False):
        ...
        if test_audio:
            audio_manager.create_audio_player(test_audio, "Test Audio", "")
    ...
    if 'stories' in st.session_state and st.session_state.stories:
        ...
        if st.button("🎧 Listen to Full Story"):
            ...
            if full_audio:
                audio_manager.create_audio_player(full_audio, story_text_english, "")
        st.markdown("---")
